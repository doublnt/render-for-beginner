...
static bool insideTriangle(int x, int y, const Vector3f* _v) {
  ...
  auto res0 = (src_point - _v[0]).cross(_v[2] - _v[0]);
  auto res1 = (src_point - _v[2]).cross(_v[1] - _v[2]);
  auto res2 = (src_point - _v[1]).cross(_v[0] - _v[1]);

  if (_v[1].x() > _v[0].x()) {
    res0 = (src_point - _v[0]).cross(_v[1] - _v[0]);
    res1 = (src_point - _v[1]).cross(_v[2] - _v[1]);
    res2 = (src_point - _v[2]).cross(_v[0] - _v[2]);
  }

  if ((res0.z() >= 0 && res1.z() >= 0 && res2.z() >= 0) ||
      (res0.z() <= 0 && res1.z() <= 0 && res2.z() <= 0)) {
    return true;
  }
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
  ...
  std::initializer_list x_list{vertex0.x(), vertex1.x(), vertex2.x()};
  auto left = static_cast<int>(std::min(x_list));
  auto right = static_cast<int>(std::max(x_list));

  std::initializer_list y_list{vertex0.y(), vertex1.y(), vertex2.y()};
  auto top = static_cast<int>(std::max(y_list));
  auto bottom = static_cast<int>(std::min(y_list));
    ...
    for (int j = bottom; j <= top; ++j) {
      if (insideTriangle(i, j, t.v)) {
        ...
        if (z_interpolated < depth_buf[index]) {
          ...
          // 只要在 距离照相机近的地方 绘制就可以啦。
          // 之前放到 if 外面，这样每次都会绘制的话，其实就会都进行绘制了。
